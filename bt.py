...
def check_bt(donor, recipient):
    ...
    """
    if str(type(donor)) == "<enum 'Bloodtype'>":
        compatible = _particular_antigen_comp(donor.value, recipient.value)
        return all(x >= 0 for x in compatible)
    elif type(donor) == str:
        d = blood_type_text.get(donor)
        r = blood_type_text.get(recipient)
        try:
            compatible = _particular_antigen_comp(d.value, r.value)
        ...
        elif recipient > 7 or recipient < 0:
            raise ValueError("Value not in range")
        else:
            compatible = _particular_antigen_comp(donor, recipient)
            return all(x >= 0 for x in compatible)
    else:
